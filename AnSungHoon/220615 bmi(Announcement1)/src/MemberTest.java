...
public class MemberTest {
	...
	private void registerMember() {
		...
		while (a) {
			...
			System.out.println(
					"< " + count + "명 등록완료" + "                        " + (10 - count) + " 명 남았습니다.(최대 10명) >");
	...
	private void wantNomal() {
		...
		case 4:
			...
			break;
		case 5:
			System.out.println("이미 bmi지수가 정상입니다.");
			break;
		case 6:
			nomalCal(6);
			break;
	...
	private void nomalCal(int index) {
		String note[] = { "0", "고도비만(3단계 비만)", "중도비만 (2단계 비만)", "경도비만 (1단계 비만)", "과체중", "정상", "저체중" };
		int cnt = 0;
		for (int i = 0; i < members.length; i++) {
			...
			if (members[i] != null) {
				if (members[i].getNote().equals(note[index])) {
					...
					if (index == 6) {
						System.out.println(members[i].getName() + "님은 bmi가 정상이 되려면 살을 "
								+ String.format("%.2f", Math.abs(members[i].getWeight() - normalWeight))
								+ "kg 증량 하시거나 키를 "
								+ String.format("%.2f", Math.abs((normalHeight - members[i].getHeight())) * 100)
								+ "cm 더 줄이십시오.");
					} else {
						System.out.println(members[i].getName() + "님은 bmi가 정상이 되려면 살을 "
								+ String.format("%.2f", members[i].getWeight() - normalWeight) + "kg 감량 하시거나 키를 "
								+ String.format("%.2f", (normalHeight - members[i].getHeight()) * 100) + "cm 더 키우십시오.");
					}
				}
			}
		}
